...
- **C-4**: Платежи только через сертифицированный банковский онлайн-эквайринг (конкретный банк пока не выбран)
...
- **Payments**: банковский онлайн-эквайринг (провайдер пока не выбран); за абстракцией `PaymentProvider`, по умолчанию mock
...
- **Интеграция с платежной системой** (банковский эквайринг)
...
- Внешние API (эквайер, SMS): retry 3 раза с backoff 1s -> 2s -> 4s
...
- **Платежные данные**: не хранятся на платформе, только token от эквайера
...
- **C-4** (Сертифицированные платежи) -> абстракция PaymentProvider готова (mock по умолчанию), токены, не храним карты; конкретный банк-эквайер пока не выбран -- IN PROGRESS
...
- [ ] Определить SLA для внешних провайдеров (банк-эквайер, Яндекс Карты)
...
2. `payment-service` (интеграция банковского эквайринга)
