...
видит ссылку на оплату (если провайдер успел её выдать) без соответствующей
брони.
...
платёж истечёт по TTL на стороне провайдера (обычно ~1 час для pending-платежей)
...
2. Реализовать в payment-service: отзыв платежа через API эквайера
   (метод отмены/возврата платежа выбранного провайдера).
3. В `CreateBooking`, при сбое `SetPaymentAndStatus`, вызывать `CancelPayment`
   до `repo.Delete`.

**Triggers for prioritisation**  
- Появление жалоб на «оплатил, но брони нет» (webhook от эквайера по
...
## [PAYMENT-WEBHOOK-SIGNATURE] — VerifySignature объявлена, но не вызывается

**File**: `services/payment-service/internal/delivery/grpc/server.go` — `HandleWebhook()`  
**Provider iface**: `services/payment-service/internal/provider/provider.go` — `VerifySignature()`  
**Proto**: `proto/payment/v1/payment.proto` — `WebhookRequest`

**Current behaviour**  
Интерфейс провайдера объявляет `VerifySignature(ctx, rawBody, headers)`, но
usecase/delivery никогда её не вызывает. Аутентичность вебхука сейчас
обеспечивается **исключительно IP-allowlist на api-gateway** — вебхук
принимается только если пришёл с серверов платёжной системы.

Для текущего mock-провайдера это некритично: реальных вебхуков он не шлёт, а
его `VerifySignature` — no-op (возвращает nil). На текущей конфигурации дыры нет.

Проблема в том, что код **выглядит** защищённым подписью, хотя проверка
отключена. Заголовки HTTP-запроса теряются на границе api-gateway →
payment-service: gRPC `WebhookRequest` несёт только `raw_body`, без headers.
Поэтому даже при желании вызвать `VerifySignature` нечего ей передать.

**Accepted residual risk**  
Пока активен mock-провайдер — нулевой (реальных платежей нет; на проде
IP-allowlist закрывает). Риск материализуется при подключении банк-эквайера,
который аутентифицирует вебхуки HMAC-подписью, а не IP (T-Bank, Sber). Если
такой провайдер подключат и забудут включить проверку подписи, любой, кто знает URL вебхука, сможет
прислать поддельный `payout.succeeded` → ложное списание со счёта платформы.
Payout-вебхуки опаснее payment-вебхуков: подделка приёма в худшем случае
ложно отметит бронь оплаченной (деньги на счету платформы), подделка выплаты —
это вывод реальных денег.

**Когда появится HMAC-провайдер**  
По текущей escrow-модели (платформа держит всё на одном счету, выплачивает
партнёрам) это произойдёт уже при подключении банковского эквайринга:
большинство российских эквайеров (T-Bank, Sber и т.п.) подписывают вебхуки
HMAC, а не аутентифицируют по IP. То есть триггер — это «выбрали банк-эквайер».

**Upgrade path**  
1. Добавить `map<string, string> headers = N;` в `WebhookRequest`
   (`proto/payment/v1/payment.proto`), `make proto-gen`.
2. В api-gateway (`internal/handler/payment.go`) пробросить **только**
   заголовки, нужные провайдеру для проверки подписи (whitelist конкретных
   header-имён — НЕ весь набор, чтобы не утянуть лишнего; учесть webhook
   logging policy из CLAUDE.md).
3. В payment-service (`delivery/grpc/server.go`) вызвать
   `provider.VerifySignature(ctx, rawBody, headers)` **до** парсинга тела,
   возвращать `codes.PermissionDenied` при несовпадении.
4. Реализовать `VerifySignature` для HMAC-провайдера (T-Bank/Sber) на этом
   же шаге — не оставлять no-op.

**Дешёвый промежуточный предохранитель (без proto-изменений)**  
Чтобы будущий разработчик не включил HMAC-провайдера молча мимо проверки:
при старте сервиса падать (или при инициализации провайдера), если у
активного провайдера `VerifySignature` — не no-op, но delivery-слой его не
вызывает. Альтернатива — тест, который проверяет, что для каждого провайдера
с реальной подписью `HandleWebhook` вызывает `VerifySignature`.

**Triggers for prioritisation**  
- Решение подключить T-Bank/Sber для выплат партнёрам (блокер этого релиза).
- Любой провайдер, аутентифицирующий вебхуки подписью вместо IP-allowlist.
