...
Для ЮKassa это корректно и достаточно: ЮKassa аутентифицирует вебхуки именно
по IP, а её реализация `VerifySignature` — no-op (возвращает nil). То есть на
текущей конфигурации дыры нет.
...
**Accepted residual risk**  
Пока активен только ЮKassa — нулевой (IP-allowlist закрывает). Риск
материализуется при подключении провайдера, который аутентифицирует вебхуки
HMAC-подписью, а не IP (T-Bank, Sber). Если такой провайдер выплат подключат
и забудут включить проверку подписи, любой, кто знает URL вебхука, сможет
...
Вероятный сценарий по текущей escrow-модели (платформа держит всё на одном
счету, выплачивает партнёрам): приём оставить на ЮKassa, а **выплаты** увести
на T-Bank ради более дешёвой комиссии на вывод. Приём денег (acquiring) менять
маловероятно. То есть триггер — это «удешевить выплаты партнёрам».
...
**Если выплаты навсегда остаются на ЮKassa** — запись можно закрыть как
won't-fix: IP-allowlist полностью покрывает модель угроз.
